...
class DataDescription:
    ...
    def showDataset(self):
        print(f"The shape of the dataset is {self.data.shape}")
        print(f"No.of rows: {self.data.shape[0]}")
        print(f"No of columns: {self.data.shape[1]}")
        while(1):
            try:
                
                rows = int(input((f"\nHow many rows(>0 & <{self.data.shape[0]}) to print? (Press -1 to go back)  ")))
                if rows == -1:
                    break
                if rows <= 0:
                ...
                if rows>self.data.shape[0]:
                    print(f"Pleas enter the number <={self.data.shape[0]}")
                    continue
